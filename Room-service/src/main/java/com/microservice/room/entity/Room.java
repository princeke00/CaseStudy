...
public class Room {
	...
	public Room(@NotNull(message="Room ID cannnot be null")@Size(min=8,message="Room ID should be minimum 8 digits")long roomId,
			@NotNull(message="Room type be null")@Size(min=4,message="Room type be minimum 4 letters")String roomType,
			@NotNull(message="Room rent cannnot be null")@Size(min=3,message="Room rent should be minimum 4 digits")double roomRent,
			@NotNull(message="Room avaialble cannnot be null")@Size(min=4,message="Room available should be minimum 4 letters and should be either 'True' or 'False'")boolean roomAvailable) {
		super();
		this.roomId = roomId;
		this.roomType = roomType;
		this.roomRent = roomRent;
