...
public class ReservationServiceImpl implements ReservationService{
	...
	public String addReservation(Reservation reservation) {
		Room room= restTemplate.getForObject("http://localhost:9002/room/get/"+reservation.getRoomId(), Room.class);
		Guest guest=restTemplate.getForObject("http://localhost:9004/guest/get/"+reservation.getGuestId(), Guest.class);
		if(room.isRoomAvailable()) {
			reservationRepository.save(reservation);
			room.setRoomAvailable(false);
			restTemplate.put("http://localhost:9002/room/update/"+reservation.getRoomId(), room);
